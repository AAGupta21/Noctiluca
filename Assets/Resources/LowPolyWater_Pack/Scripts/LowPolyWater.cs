...
namespace LowPolyWater
{
    public class LowPolyWater : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            if(Manager.InWater)
            {
                if (gameObject.transform.position.x != camera.transform.position.x)
                {
                    Vector3 pos = new Vector3(camera.transform.position.x + 46.78f, gameObject.transform.position.y, gameObject.transform.position.z);
